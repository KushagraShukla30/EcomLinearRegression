...
pairs(ecomdata[c("Avg..Session.Length", 
                 "Time.on.App", 
                 "Time.on.Website", 
...
set.seed(1)
row.number <- sample(1:nrow(ecomdata), 0.8*nrow(ecomdata))
train <- ecomdata[row.number,]
test <- ecomdata[-row.number,]
